...
local function swait(num)
  if num == 0 or num == nil then
    game:GetService("RunService").PostSimulation:Wait(0)
  else
    for i = 0, num do
      game:GetService("RunService").PostSimulation:Wait(0)
    end
...
local Hat = Character:FindFirstChild("BladeMasterAccessory")
local Hat2 = Character:FindFirstChild("ShadowBladeMasterAccessory")
if Hat then
	Global.AlignPart(Hat.Handle, handle, Vector3.new(1.6, 1.5, 0), Vector3.new(0, 0, 135))
...
if Hat2 then
	Global.AlignPart(Hat2.Handle, handle2, Vector3.new(1.6, 1.5, 0), Vector3.new(0, 0, 135))
...
mus.Name = "mus"
mus.SoundId = getsynasset("GelatekHub/Scripts/DualUltima.mp3")
mus.Looped = true
mus.Volume = 1
mus:Play()
local colrr = Color3.new()
local maincol = Color3.new()
local TextLabel
do
	...
	table.insert(Events, game:GetService("RunService").RenderStepped:connect(function()
		local CurrentLoudness = mus.PlaybackLoudness
	...
	local showup = Instance.new("BillboardGui"); do
		showup.Size = UDim2.new(3, 0, 1, 0)
		showup.ExtentsOffset = Vector3.new(-3, 3, 0)
		showup.Parent = Head
	end;
	TextLabel = Instance.new("TextLabel"); do
		TextLabel.BackgroundTransparency = 1
		TextLabel.Text = "Duelo Ultima"
		TextLabel.Size = UDim2.new(3,0,1,0)
		TextLabel.TextColor3 = Color3.fromRGB(255,255,255)
		TextLabel.TextScaled = true
		TextLabel.Parent = showup
	end;
	task.spawn(function()
		while task.wait(0.004) do
			...
			TextLabel.TextColor3 = colrr
			if (math.random(1,5) == 1) then
			TextLabel.Text = "Duel0 UlTim4"
			elseif (math.random(1,5) == 2) then
			TextLabel.Text = "Du31o Ult1m@"
			elseif (math.random(1,5) == 3) then
			TextLabel.Text = "Du31o Ult1m@"
			elseif (math.random(1,5) == 4) then
			TextLabel.Text = "6u3lo U1tInna"
...
coroutine.resume(coroutine.create(function()
local Eventussy2
Eventussy2 = game:GetService("RunService").Stepped:Connect(function()
if Global.Stopped == true then Eventussy2:Disconnect() end
local Hue = tick() % 2/2
colrr = Color3.fromHSV(Hue, 1, 1)
updateCol2(colrr)
maincol = Part.Color
...
Neck.C0=clerp(Neck.C0,necko*angles(math.rad(10),math.rad(0),math.rad(0)),.4)
