-- notice: this is my old convert, just decided to fix it up and done
...
local function so(id,par,pit,vol)
...
local function destroyshit(p0,p1)
	p0.Attachment:Destroy()
	p1.Attachment:Destroy()
	p0.AlignPosition:Destroy()
	p0.AlignOrientation:Destroy()
end
local Hat1 = chara:FindFirstChild('Type-49 Abomindation Back Accessory')
local Hat2 = chara:FindFirstChild('Hussar SabreAccessory')
if Hat1 then
	Align(Hat1.Handle,Handaru,Vector3.new(0.7, 0, -0.2),Vector3.new(0,0,65))
...
if Bullet then
	...
	table.insert(Events, game:GetService("RunService").PostSimulation:Connect(function()
		if shoote then
			...
			Pos.Position = roottohandle.Position
		end
	end))
end
...
local EffectModel = Create("Model"){
Parent = chara,
Name = "Effects",
}
...
local function GetDudesTorso(c)
...
local function BodyVel(part,faws)
...
local function Dmg(dude,dmg,env,faws,trip,efftyp,toim)

end

local function MagnitudeDmg(par,magni,dmg,env,faws,trip,efftyp,toim)

end

local function CamShake(par,magni,env,dur)

end

local function puncheff(par)
...
if Hat2 then
Align(Hat2.Handle,KnifeH,Vector3.new(-0.1,0,0),Vector3.new(90,0,0))
end
local function wepknifestab()
	
if Hat2 then
	destroyshit(Hat2.Handle,KnifeH)
	Align(Hat2.Handle,Knife2,Vector3.new(0,0,-0.6),Vector3.new(90,0,0))
end
attack = true
...
attackwithknife = false
if Hat2 then
destroyshit(Hat2.Handle,Knife2)
Align(Hat2.Handle,KnifeH,Vector3.new(-0.1,0,0),Vector3.new(90,0,0))
end
end
...
table.insert(Events,game:GetService("RunService").RenderStepped:connect(function()
...
Eventussy = game:GetService("RunService").Stepped:Connect(function()
	if Global.Stopped == true then
		for i,v in pairs(chara.Torso:GetChildren()) do
