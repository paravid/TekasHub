    ...
    if hum ~= nil then
      ...
      if head ~= nil then
        ...
        if magni >= mag and c.Name ~= plr.Name then
          Damage(head, head, 0, 0, knock, Type, root, 0.1, "http://www.roblox.com/asset/?id=0", 1.2)
        end
      end
    end
  end
...
Music.Volume = 2.5
Music.SoundId = getsynasset("GelatekHub/Scripts/Ender.mp3")
Music.Looped = true
...
function Destruction()
	...
	for i = 0, 10, 0.1 do
		...
		for i, v in pairs(FindNearestHead(ba.CFrame.p, 14.5)) do
			if v:FindFirstChild("Head") then
				if Bullet then funnyfunction(v) end
			end
		end
	end
	a:Destroy()
...
table.insert(Events,mouse.KeyDown:connect(function(key)
	if attack == false then
		...
		elseif key == 'r' then
			BURN_IN_HELL()
		elseif key == 't' then
			chatfunc("HM, HM, HAHAHAHAHAHA (kys)", BrickColor.random().Color)
...
local eventoslos
eventoslos = game:GetService("RunService").Stepped:Connect(function()
	if Global.Stopped == true then
		eventoslos:Disconnect()
	end
	swait()
for i, v in pairs(NeonParts) do
